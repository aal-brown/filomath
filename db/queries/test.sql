...
SELECT resources.id, resources.user_id, users.username, resources.title, resources.description, resources.resource_url, resources.thumbnail_url, resources.date, (SELECT count(likes.*) from likes WHERE likes.resource_id = resources.id) as likes, avg(t.rating) as global_rating, /* (SELECT ratings.rating from ratings where ratings.resource_id = resources.id and ratings.user_id = 32 group by resources.id, ratings.rating) as user_rating, */ categories.category
  FROM resources
  JOIN users ON resources.user_id = users.id
  LEFT JOIN ratings AS t ON resources.id = t.resource_id
  LEFT JOIN likes ON resources.id = likes.resource_id
  LEFT JOIN resource_categories ON resources.id = resource_categories.resource_id
  LEFT JOIN categories ON resource_categories.category_id = categories.id
  WHERE likes.user_id = 32 AND resources.user_id != 32
  GROUP BY resources.id, users.id, categories.category;


/* Testing. FIXED needed LEFT JOINs */
...
/* This query is messing up. Debugging. SEEMS TO BE OK */
/*   SELECT resources.id, resources.user_id, users.username, resources.title, resources.description, resources.resource_url, resources.thumbnail_url, resources.date, (SELECT count(likes.*) from likes WHERE likes.resource_id = resources.id) as likes, avg(t.rating) as global_rating, (SELECT ratings.rating from ratings where ratings.resource_id = resources.id and ratings.user_id = 32 group by resources.id, ratings.rating) as user_rating, categories.category
  FROM resources
  JOIN users ON resources.user_id = users.id
  LEFT JOIN ratings AS t ON resources.id = t.resource_id
...
/* This query is possibly messing up. Debugging. SEEMS TO BE OK */
/*  SELECT resources.id, resources.user_id, users.username, resources.title, resources.description, resources.resource_url, resources.thumbnail_url, resources.date, (SELECT count(likes.*) from likes WHERE likes.resource_id = resources.id) as likes, avg(t.rating) as global_rating, (SELECT ratings.rating from ratings where ratings.resource_id = resources.id and ratings.user_id = 33 group by resources.id, ratings.rating) as user_rating, categories.category
  FROM resources
  JOIN users ON resources.user_id = users.id
  LEFT JOIN ratings AS t ON resources.id = t.resource_id
  LEFT JOIN likes ON resources.id = likes.resource_id
